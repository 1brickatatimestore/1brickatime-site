...
export default function MinifigsByThemePage(props: Props) {
  const {
    items, count, page, limit, q, cond, onlyInStock, minPrice, maxPrice, theme, sort, themes,
  } = props
          ...
          <>
            ...
            {pages > 1 && (
              <nav className="pager" aria-label="Pagination">
                <Link className="pbtn" href={buildHref({ page: Math.max(1, page - 1), limit, q, cond, onlyInStock, minPrice, maxPrice, theme, sort })} aria-disabled={page <= 1}>
                  ← Prev
                </Link>
                <span className="pmeta">Page {page} / {pages}</span>
                <Link className="pbtn" href={buildHref({ page: Math.min(pages, page + 1), limit, q, cond, onlyInStock, minPrice, maxPrice, theme, sort })} aria-disabled={page >= pages}>
                  Next →
                </Link>
              </nav>
            )}
...
function buildHref(opts: {
  ...
  minPrice?: number | null; maxPrice?: number | null; theme?: string; sort?: string
}) {
  const p = new URLSearchParams()
  ...
  if (opts.minPrice != null && opts.minPrice !== undefined && String(opts.minPrice)) p.set('minPrice', String(opts.minPrice))
  if (opts.maxPrice != null && opts.maxPrice !== undefined && String(opts.maxPrice)) p.set('maxPrice', String(opts.maxPrice))
...
export async function getServerSideProps(ctx: any) {
  ...
  if (themesRes.ok) {
    ...
    themes = Array.isArray(t.options) ? t.options : []
  }

  return { props: { items, count, page, limit, q, cond, onlyInStock, minPrice, maxPrice, theme, sort, themes } }
}
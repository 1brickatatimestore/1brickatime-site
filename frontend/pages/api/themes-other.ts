...
const containsKeys = Object.keys(OV.mapContains ?? {}).sort(
...
export default async function handler(
  ...
  try {
    ...
    const onlyInStock =
      req.query.onlyInStock === "1" || req.query.onlyInStock === "true";
    ...
    const cursor = Product.find(match, { _id: 0, name: 1, itemNo: 1 })
      .lean()
      .cursor();
    ...
    const topPrefixes = Array.from(byPrefix.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    res
      .status(500)
      .json({ ok: false, error: "themes_other_failed", message: err?.message });
  }
}

...
const BANK = {
  name:
    process.env.NEXT_PUBLIC_BANK_DEPOSIT_NAME ||
    process.env.BANK_DEPOSIT_NAME ||
    "",
  bsb:
    process.env.NEXT_PUBLIC_BANK_DEPOSIT_BSB ||
    process.env.BANK_DEPOSIT_BSB ||
    "",
  account:
    process.env.NEXT_PUBLIC_BANK_DEPOSIT_ACCOUNT ||
    process.env.BANK_DEPOSIT_ACCOUNT ||
    "",
  hint:
    process.env.NEXT_PUBLIC_BANK_DEPOSIT_REFERENCE_HINT ||
    process.env.BANK_DEPOSIT_REFERENCE_HINT ||
    "",
};
...
export default function ThankYouPage() {
    ...
    if (
      ...
      (async () => {
        try {
          ...
          const r = await fetch(
            `/api/checkout/paypal-capture?orderId=${encodeURIComponent(info.orderId)}`,
            {
              method: "POST",
              signal: abort.signal,
  ...
  const title =
    info.status === "success"
      ? `Thank you — 1 Brick at a Time`
      : info.status === "cancelled"
        ? `Order cancelled — 1 Brick at a Time`
        ...
        {info.status === "success" ? (
          <>
            ...
            <p style={{ margin: "0 0 16px", color: "#333" }}>
              ...
              {info.orderId ? ` (ref: ${info.orderId})` : ""}.
              {info.total ? ` Total: ${info.total}` : ""} You’ll get an email
              shortly.
            </p>

            {info.provider === "paypal" && <CaptureBanner cap={cap} />}

            {info.provider === "bank" &&
              (BANK.name || BANK.bsb || BANK.account) && (
                <section
                  style={{
                ...
                >
                  ...
                  <div style={{ lineHeight: 1.8 }}>
                    ...
                    {info.orderId && (
                      <div>
                        <strong>Reference:</strong> {info.orderId}
                      </div>
                    )}
                    {BANK.hint && (
                      <div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <Link href="/minifigs?type=MINIFIG&limit=36">
                <button style={btn()}>Continue shopping</button>
              </Link>
              <Link href="/checkout">
                <button style={btn("ghost")}>View cart/receipt</button>
              </Link>
            </div>
          </>
        ) : info.status === "cancelled" ? (
          <>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <Link href="/checkout">
                <button style={btn()}>Return to checkout</button>
              </Link>
              <Link href="/minifigs?type=MINIFIG&limit=36">
                <button style={btn("ghost")}>Keep browsing</button>
              </Link>
            </div>
          </>
        ) : (
          <>
            ...
            <p style={{ margin: "0 0 16px", color: "#333" }}>
              We’re not sure yet—try refreshing this page, or head back to
              checkout.
            </p>
            <div style={{ display: "flex", gap: 12 }}>
              <Link href="/checkout">
                <button style={btn()}>Go to checkout</button>
              </Link>
              <Link href="/minifigs?type=MINIFIG&limit=36">
                <button style={btn("ghost")}>Browse minifigs</button>
              </Link>
            </div>
          </>
        )}
...
function CaptureBanner({ cap }: { cap: CaptureState }) {
  ...
  if (cap.kind === "capturing") {
    return (
      <div style={banner("#fff3cd", "#856404")}>Capturing PayPal payment…</div>
    );
  }
  if (cap.kind === "ok") {
    return (
  ...
  return (
    <div style={banner("#f8d7da", "#721c24")}>
      PayPal capture failed. You can retry from your account, or contact us. (
      {cap.message})
    </div>
  );
}

...
export default function MinifigsPage(props: Props) {
  const { items, count, page, limit, q, cond, minPrice, maxPrice, sort } =
    props;
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    p.set("page", "1");
    p.set("limit", String(fd.get("limit") || limit || 36));
    ...
    <>
      ...
      <main className="wrap">
        <form className="filters" onSubmit={submit}>
          ...
          <Link className="btnGhost" href="/minifigs">
            Clear
          </Link>
                ...
                return (
                  <article key={id} className="card">
                    <Link
                      className="clickArea"
                      href={detailsHref}
                      aria-label={`Open ${p.name || p.itemNo}`}
                    />

                    <div className="imgBox" title="Open details">
                      {p.imageUrl ? (
                        <Image
                          src={p.imageUrl}
                    ...
                    <div className="row">
                      <Link className="details" href={detailsHref}>
                        Details
                      </Link>
                    </div>
                  </article>
      ...
      <style jsx>{`
        ...
        .clickArea {
          position: absolute;
          inset: 0 0 64px 0; /* whole card except button row */
          z-index: 1;
        }
        .imgBox {
          position: relative;
...
export async function getServerSideProps(ctx: any) {
  ...
  return {
    props: { items, count, page, limit, q, cond, minPrice, maxPrice, sort },
  };
}

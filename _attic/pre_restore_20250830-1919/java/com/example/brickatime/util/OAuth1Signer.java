...
/**
 * Minimal OAuth 1.0a signer for BrickLink.
 * Usage:
 *   OAuth1Signer s = new OAuth1Signer(consumerKey, consumerSecret, token, tokenSecret);
 *   String auth = s.buildAuthHeader("GET", "https://api.bricklink.com/api/store/v1/inventories?itemType=MINIFIG", null);
 */
public class OAuth1Signer {
    ...
    public OAuth1Signer(String consumerKey, String consumerSecret, String token, String tokenSecret) {
        ...
        this.consumerSecret = Objects.requireNonNull(consumerSecret, "consumerSecret");
        this.token = Objects.requireNonNull(token, "token");
        this.tokenSecret = Objects.requireNonNull(tokenSecret, "tokenSecret");
    }

    /** Builds the OAuth Authorization header value. queryParams may be null. */
    public String buildAuthHeader(String method, String url, Map<String, String> queryParams) {
        ...
        String base = method.toUpperCase() + "&" + pct(urlWithoutQuery(url)) + "&" + pct(normalized(all));
        ...
        h.append(kv("oauth_consumer_key", consumerKey)).append(", ")
    ...
    private String kv(String k, String v) { return k + "=\"" + pct(v) + "\""; }
